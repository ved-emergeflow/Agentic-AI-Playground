...
def get_google_model(model):
    """
    Returns Google Model wrapped by Langchain
    :param model:
    :return: BaseModel
    """
...
def create_research(runtime: ToolRuntime[None | RLAgentState]) -> Command:
    """
    This tool calls another agent who creates research based on all the gathered information from tools
    :return: Command
    """
...
def hitl(state: RLAgentState):
    ...
    if state['approved'] or 'approved' not in state:
        ...
        changes = interrupt(
            f"This is the current research: {output}. "
            f"Please add changes or type 'Approved' if no changes are required."
        )
...
def get_google_agent(checkpointer):
